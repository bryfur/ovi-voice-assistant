...
func serve(t *testing.T, files map[string][]byte) *int {
	...
	t.Cleanup(srv.Close)
	oldURL, oldDir := BaseURL, Dir
	BaseURL, Dir = srv.URL+"/", t.TempDir()
	t.Cleanup(func() { BaseURL, Dir = oldURL, oldDir })
	return &hits
}
...
func TestEnsureExtractsAndCaches(t *testing.T) {
	...
	dir, err := Ensure(TTS, "pack")
	dir2, err2 := Ensure(TTS, "pack")
	...
	if m, err := Find(dir, "model.int8.onnx", "model.onnx"); err != nil || filepath.Base(m) != "model.int8.onnx" {
		t.Fatalf("Find = %q, %v", m, err)
	}
	if _, err := Find(dir, "nope*.onnx"); err == nil {
...
func TestEnsureMissingPack(t *testing.T) {
	...
	if _, err := Ensure(ASR, "missing"); err == nil {
		t.Fatal("expected error")
	}
	if _, err := os.Stat(filepath.Join(Dir, ".tmp-missing")); err == nil {
...
func TestEnsureFile(t *testing.T) {
	...
	p1, err := EnsureFile(ASR, "silero_vad.onnx")
	p2, _ := EnsureFile(ASR, "silero_vad.onnx")
