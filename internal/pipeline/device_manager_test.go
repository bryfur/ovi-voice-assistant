...
func TestSingleDeviceNoArbitration(t *testing.T) {
	...
	transports[0].eventCB(device.EventWakeWord, []byte("w"))
	<-pl.started
...
func TestMultiDeviceArbitrationHighestScoreWins(t *testing.T) {
	...
	transports[0].eventCB(device.EventWakeWord, payload(1000, 100)) // 10000
	transports[1].eventCB(device.EventWakeWord, payload(3000, 100)) // 30000 → winner
	transports[2].eventCB(device.EventWakeWord, payload(500, 100))  // 5000
	pending := m.PendingCandidates()
	<-pl.started
	waitFor(t, func() bool {
		return transports[0].hasEvent(device.EventWakeAbort) && transports[2].hasEvent(device.EventWakeAbort)
	})

	if pending != 3 || pl.runCount() != 1 || !m.MultiDevice() {
		t.Fatalf("pending=%d runs=%d", pending, pl.runCount())
	}
	if transports[1].hasEvent(device.EventWakeAbort) || !m.Connections()[1].TaskRunning() {
