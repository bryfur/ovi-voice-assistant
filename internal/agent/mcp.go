// Minimal Model Context Protocol client over stdio: newline-delimited
// JSON-RPC 2.0 to a subprocess with initialize, tools/list and tools/call.
package agent
...
// mcpProtocolVersion is the MCP revision we advertise.
const mcpProtocolVersion = "2025-06-18"

// MCPServer describes how to launch an MCP server.
...
func (c MCPServer) DisplayName() string {
	if c.Name != "" {
		return c.Name
	}
	if len(c.Args) > 0 {
...
// parseMCPServers parses a JSON array of MCPServer, or "@path" to a file.
...
// newMCPClient creates an unstarted client.
func newMCPClient(cfg MCPServer) *mcpClient {
	return &mcpClient{cfg: cfg, pending: map[int64]chan rpcMessage{}, done: make(chan struct{})}
}

// Name returns the server's display name.
func (c *mcpClient) Name() string { return c.cfg.DisplayName() }
...
func (c *mcpClient) readLoop(stdout io.Reader) {
	...
	for scanner.Scan() {
		...
		if err := json.Unmarshal([]byte(line), &msg); err != nil {
			slog.Debug("MCP: non-JSON line from server", "name", c.Name(), "line", truncate(line, 200))
			continue
		}
		if msg.Method != "" {
...
func (c *mcpClient) handleServerMessage(msg rpcMessage) {
