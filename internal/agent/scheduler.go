...
// RunPromptFunc runs a prompt through the agent and returns the response.
type RunPromptFunc func(ctx context.Context, prompt string) (string, error)

// AnnounceFunc speaks text on all devices.
type AnnounceFunc func(ctx context.Context, text string) error

// Scheduler runs persistent cron automations in the background.
type Scheduler struct {
	path      string
	runPrompt RunPromptFunc
...
func NewScheduler(path string, runPrompt RunPromptFunc, announce AnnounceFunc) *Scheduler {
