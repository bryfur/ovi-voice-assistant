// Package device manages connections to voice devices and multi-device
...
// Pipeline is the voice pipeline surface a connection needs.
type Pipeline interface {
	Run(ctx context.Context, output audio.PipelineOutput, mic <-chan []byte, actx *agent.Context) bool
	Announce(ctx context.Context, output audio.PipelineOutput, text string)
	ResetHistory()
}
...
type DeviceConnection struct {
	...
	output    *pipeline.EncodingOutput
	onWake    WakeCallback
	...
	musicOutput *pipeline.EncodingOutput
...
// Options configure a DeviceConnection.
type Options struct {
	OnWake     WakeCallback
	Name       string
	MusicGroup *music.MusicGroup
...
func NewDeviceConnection(t transport.DeviceTransport, spkCodec codec.AudioCodec, p Pipeline, settings *config.Settings, opts Options) *DeviceConnection {
	name := opts.Name
	if name == "" {
		name = t.String()
	}
	c := &DeviceConnection{
		...
		pipeline:   p,
		settings:   settings,
		output:     pipeline.NewEncodingOutput(t, spkCodec),
		onWake:     opts.OnWake,
...
func (c *DeviceConnection) setupDevice() error {
	...
	musicCodec, err := codec.CreateNamed(c.settings.Transport.Codec, 48000, 2, codec.LC3MusicNByte)
	if err != nil {
...
func (c *DeviceConnection) onEvent(event transport.EventType, payload []byte) {
	switch event {
	case transport.EventWakeWord:
		c.handleWake(payload)
	case transport.EventMicConfig:
...
func (c *DeviceConnection) handleMicConfig(payload []byte) {
	...
	if preferred != string(name) {
		pref, err := codec.CreateNamed(preferred, rate, 1, 0)
		if err == nil {
			buf := make([]byte, 7)
...
func (c *DeviceConnection) runMusic(ctx context.Context, player *music.MusicPlayer) {
	out := c.musicOutput
	out.Reset()
	if err := out.SendEvent(ctx, transport.EventTTSStart, nil); err != nil {
		return
	}
...
func (c *DeviceConnection) AbortWake() {
	...
	_ = c.transport.SendEvent(transport.EventWakeAbort, nil)
