...
type TransportFactory func(dev config.DeviceConfig) transport.DeviceTransport
...
func NewDeviceManager(devices []config.DeviceConfig, settings *config.Settings, p Pipeline, ttsRate int, factory TransportFactory) (*DeviceManager, error) {
	if factory == nil {
		factory = func(dev config.DeviceConfig) transport.DeviceTransport {
			return transport.NewWiFiTransport(dev.Host, dev.Port, dev.EncryptionKey)
		}
	}
	...
	for _, dev := range devices {
		...
		m.connections = append(m.connections, NewDeviceConnection(
			...
			Options{OnWake: onWake, Name: dev.Host, MusicGroup: m.musicGroup},
		))
	}
	return m, nil
}
