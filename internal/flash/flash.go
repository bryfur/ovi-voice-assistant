// Package flash compiles and uploads ESPHome firmware interactively.
package flash
...
// DeviceName extracts the ESPHome device name from a YAML config.
...
// Run drives the interactive device flashing flow.
func Run(c *console.IO) {
	c.Println()
	...
	if err == nil {
		...
		if name := DeviceName(selected.Path); name != "" {
...
func promptSecrets(c *console.IO, path string) bool {
...
func scanAndAddDevice(c *console.IO, name string) {
	...
	for _, d := range devices {
		if strings.HasPrefix(d.Name, name) {
			c.Print("  Found: %s (%s)\n", d.Name, d.IP)
			if err := config.AddDevicesToConfig([]string{d.Host}, ""); err != nil {
