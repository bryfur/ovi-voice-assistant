...
func (n *nemotron) Load() error {
	...
	dir, err := ensurePack(asrRelease, fmt.Sprintf(nemotronPack, chunk))
	if err != nil {
		return err
	}
	...
	join, err := findFile(dir, "joiner*.int8.onnx", "joiner*.onnx")
	if err != nil {
		return err
	}
	tokens, err := findFile(dir, "tokens.txt")
	if err != nil {
		return err
	}
	rc := sherpa.OnlineRecognizerConfig{DecodingMethod: "greedy_search"}
	rc.FeatConfig = sherpa.FeatureConfig{SampleRate: micRate, FeatureDim: 128}
...
func (n *nemotron) Listen(ctx context.Context, mic <-chan []byte, onSpeech func()) (string, error) {
	...
	seg, err := listen(ctx, mic, n.vad, onSpeech, func(samples []float32) {
		stream.AcceptWaveform(micRate, samples)
