...
func TestNewTTSProviders(t *testing.T) {
	for _, p := range []string{"kokoro", "piper"} {
		if _, err := NewTTS(config.TTSConfig{Provider: p}, 16000); err != nil {
			t.Fatal(err)
		}
	}
	if _, err := NewTTS(config.TTSConfig{Provider: "bogus"}, 16000); err == nil {
...
func TestSynthesizeBeforeLoad(t *testing.T) {
	s, _ := NewTTS(config.TTSConfig{Provider: "kokoro"}, 16000)
