...
func listen(ctx context.Context, mic <-chan []byte, v vad, onSpeech func(), feed func([]float32)) ([]float32, error) {
	...
	for {
		...
		if seg, ok := v.Segment(); ok {
			slog.Info("End of speech", "secs", float64(len(seg))/micRate)
			return seg, nil
		}
...
func newSileroVAD(minSilence float64) (*sileroVAD, error) {
	...
	path, err := ensureFile(asrRelease, "silero_vad.onnx")
	if err != nil {
		return nil, err
	}
	cfg := sherpa.VadModelConfig{SampleRate: micRate, NumThreads: 1, Provider: "cpu"}
