...
const (
	asrRelease = "asr-models"
	ttsRelease = "tts-models"
)
...
// ensurePack returns the directory of an extracted pack, downloading
// <release>/<name>.tar.bz2 if it is not cached yet.
func ensurePack(release, name string) (string, error) {
...
// findFile returns the first file in dir matching one of the globs, in order.
func findFile(dir string, globs ...string) (string, error) {
	for _, g := range globs {
