...
// NewTTS creates the configured synthesizer, outputting PCM at rate (0 =
// model native).
func NewTTS(cfg config.TTSConfig, rate int) (TTS, error) {
	switch cfg.Provider {
	case "kokoro", "piper":
	default:
...
func (s *sherpaTTS) kokoro() (sherpa.OfflineTtsKokoroModelConfig, error) {
	...
	dir, err := ensurePack(ttsRelease, kokoroPack)
	if err != nil {
		return c, err
	}
	model, err := findFile(dir, "model.onnx", "model.int8.onnx")
	if err != nil {
		return c, err
	}
...
func (s *sherpaTTS) piper() (sherpa.OfflineTtsVitsModelConfig, error) {
	...
	dir, err := ensurePack(ttsRelease, "vits-piper-"+name)
	if err != nil {
		return c, err
	}
	model, err := findFile(dir, "*.onnx")
	if err != nil {
		return c, err
	}
