...
- **TTS**: [Piper](https://github.com/rhasspy/piper) ONNX voices. Runs on CPU. Models auto-download on first use.
...
Flash the device (ESPHome is included as a dev dependency):

```bash
uv run --group dev esphome run esphome/voice-pe.yaml
```
...
```bash
uv run ova voice-pe-XXXX.local
```
...
| `OVA_STT_MODEL` | `base.en` | faster-whisper model (`tiny.en`, `base.en`, `small.en`, `medium.en`) |
| `OVA_TTS_MODEL` | `en_US-lessac-medium` | Piper voice name |
| `OVA_STT_PROVIDER` | `whisper` | STT provider |
| `OVA_TTS_PROVIDER` | `piper` | TTS provider |
...
```
uv run ova voice-pe-XXXX.local::KEY
...
src/open_voice_assistant/
    ...
    voice_assistant_pipeline.py  STT -> Agent -> TTS orchestration
    discovery.py             mDNS device discovery
    agent/assistant.py       OpenAI Agents SDK wrapper
    stt/                     STT interface + faster-whisper implementation
    tts/                     TTS interface + Piper implementation
esphome/
