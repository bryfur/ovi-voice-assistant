    ...
    async def transcribe_stream(
        ...
        async for chunk in audio_chunks:
            ...
            if not speech_detected and elapsed > NO_SPEECH_TIMEOUT_S:
                logger.info(
                    "No speech detected after %.0fs, giving up", NO_SPEECH_TIMEOUT_S
                )
                break
            ...
            while len(vad_buf) >= chunk_bytes:
                ...
                samples = (
                    np.frombuffer(vad_chunk, dtype=np.int16).astype(np.float32)
                    / 32768.0
                )
                audio_with_context = np.concatenate([context[0], samples]).reshape(
                    1, -1
                )
